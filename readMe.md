## 记录下前端学习路上的小点点

### 目录

#### 2020-01

- [基于 prerender-spa-plugin 实现预渲染(仅适用于静态网站)](https://github.com/luoquanquan/learn-fe/tree/master/vue-ssr/prerender)
- [自己写一个骨架屏插件](https://github.com/luoquanquan/learn-fe/blob/master/pwa/pwa-vue/my-skeleton-plugin.js)
- [基于 vue 的 pwa 应用](https://github.com/luoquanquan/learn-fe/tree/master/pwa/pwa-vue)
- [pwa & manifest.json 配置 & serviceWorker 配置](https://github.com/luoquanquan/learn-fe/tree/master/pwa)
- [http 缓存 (304 Or 200)](https://github.com/luoquanquan/learn-fe/tree/master/http/cache)
- [手写 ejs 模板引擎](https://github.com/luoquanquan/learn-fe/blob/master/simple-ejs/)
- [Cookie(签名) & 加密(md5 sha256) & session](https://github.com/luoquanquan/learn-fe/blob/master/simple-cookie/)
- [手写实现 instanceof](https://github.com/luoquanquan/learn-fe/blob/master/simple-instanceof/index.js)
- [hash & history 前端路由](https://github.com/luoquanquan/learn-fe/tree/master/fe-router)
- [双向数据绑定](https://github.com/luoquanquan/learn-fe/tree/master/data-binding)
- [手写 bind & call & apply](https://github.com/luoquanquan/learn-fe/tree/master/customize-this)
- [Server Cookie 操作](https://github.com/luoquanquan/learn-fe/blob/master/simple-cookie/server.js)
- [apiServer & CORS 跨域 & 携带 Cookie](https://github.com/luoquanquan/learn-fe/blob/master/api-server)
- [大文件分块上传 & 秒传 & 断点续传](https://github.com/luoquanquan/learn-fe/tree/master/file-upload/split-upload)
- [各种方式实现的文件上传](https://github.com/luoquanquan/learn-fe/tree/master/file-upload)
- [原生 node 实现静态文件服务](https://github.com/luoquanquan/learn-fe/blob/master/http/static-server.js)
- [http 模块基础使用](https://github.com/luoquanquan/learn-fe/tree/master/http)
- [使用流读写文件-速率控制](https://github.com/luoquanquan/learn-fe/blob/master/node-fs/read%2Bwrite-stream.js)
- [使用可写流写入文件](https://github.com/luoquanquan/learn-fe/blob/master/node-fs/write-stream.js)
- [使用可读流读取文件](https://github.com/luoquanquan/learn-fe/blob/master/node-fs/read-stream.js)
- [模拟一个 events 库](https://github.com/luoquanquan/learn-fe/blob/master/node-fs/event.js)
- [fs 文件基础读写](https://github.com/luoquanquan/learn-fe/blob/master/node-fs/fs.js)
- [Buffer 基础使用](https://github.com/luoquanquan/learn-fe/blob/master/buffer/buffer.js)
- [实现 base64 编码, 解释编码后文件变大原因](https://github.com/luoquanquan/learn-fe/blob/master/buffer/base64.js)
- [node 事件环](https://github.com/luoquanquan/learn-fe/blob/master/event-loop/node/index.js)
- [浏览器事件环](https://github.com/luoquanquan/learn-fe/tree/master/event-loop/browser)
- [generator 使用 & 自己实现的 co 库](https://github.com/luoquanquan/learn-fe/blob/master/simple-generator)

#### 2019-12

- [手写一个发布-订阅(中介者)模式](https://github.com/luoquanquan/learn-fe/tree/master/simple-agency)
- [手写一个观察者模式](https://github.com/luoquanquan/learn-fe/tree/master/simple-observer)
- [手写一个 promise](https://github.com/luoquanquan/learn-fe/tree/master/simple-promise)
- [手写一个 express](https://github.com/luoquanquan/learn-fe/tree/master/simple-express)
